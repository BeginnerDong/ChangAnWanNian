		...
		<view class="pageBox pb-4" :style="{marginTop:44+statusBar + 'px'}">
			<view class="myHead text-center d-flex a-center flex-column">
				...
				<view class="d-flex a-center font-26 mt-2">
					<image style="width:42rpx; height: 33rpx;margin-right: 10rpx;" src="../../static/images/yuanbao-img.png" mode=""></image>元宝(个)
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			uni.showLoading();
			const callback = (res) => {
				self.yb = parseFloat(parseFloat(uni.getStorageSync('user_info').info.yb_score) + parseFloat(uni.getStorageSync(
					'user_info').info.yb_today)).toFixed(2);
